					...
					for (unsigned w2 = 0; w2 < in.Warehouses(); w2++)
						if (w1 != w2)
						{
							...
							if (sol->Incompatibilities(w2, s1) <= 1)
								...
								{
									unsigned s2 = *it3;
									if (s1 != s2 && ((!sol->Incompatibilities(w1, s2) && !sol->Incompatibilities(w2, s1)) || (sol->Incompatibilities(w1, s2) == 1 && in.Incompatible(s1, s2))) 
										&& sol->Supply(s1, w1) <= sol->ResidualCapacity(w2) + sol->Supply(s2, w2) && sol->Supply(s2, w2) <= sol->ResidualCapacity(w1) + sol->Supply(s1, w1))
									{
